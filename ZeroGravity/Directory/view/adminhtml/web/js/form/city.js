    ...
    (function ($) {
        $(document).ready(function () {
            ...
            jQuery('.city-select').on('change',function(){
                UpdateRegion(jQuery(this));
            });
        });
    })(jQuery);

    function UpdateRegion(_el) {
        var cityId = _el.attr('id');
        var sectionPrefix = cityId.replace('city', "");
        var _countryId=sectionPrefix+'country_id';
        var _region=sectionPrefix+'region';
        var _regionId=sectionPrefix+'region_id';
     // console.log(sectionPrefix+' '+cityId+' '+_countryId+' '+_region+' '+_regionId);
        if ('order-shipping_address_' !== sectionPrefix) {
            var updater = new RegionUpdater(_countryId, _region, _regionId, billingRegions, 'disable');
            updater.update();
        }
    }
    /**
        ...
        update: function () {
            ...
            if (this.regions[this.countryEl.value]) {
             ...
             //   if (this.lastCountryId != this.countryEl.value) { //eslint-disable-line eqeqeq
                    ...
                    var countrystr = jQuery(this.countryEl).attr('id');
                  //  var _section=countrystr.replace('country_id', "");
                    var cityId = countrystr.replace('country_id', "") + 'city';
                    var _city = jQuery('#' + cityId).val();
                    console.log(cityId+' '+_city);
                    if(_city){
                        for (regionId in this.regions[this.countryEl.value]) { //eslint-disable-line guard-for-in

                            region = this.regions[this.countryEl.value][regionId];

                            if(region.city===_city) {
                                // console.log(this.regions[this.countryEl.value]);
